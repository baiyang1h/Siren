...
#define BTN_ID_LSM6DSL_INT        0  /**< ID of button used to sample lsm6dsl interrupt. */
 ...
 * @return A propagated error code.
 */
static uint32_t smartsock_buttons_configure(void)
{
    uint32_t err_code;

    err_code = bsp_event_to_button_action_assign(BTN_ID_LSM6DSL_INT,
                                                 BSP_BUTTON_ACTION_PUSH,
                                                 BSP_EVENT_LSM6DSL_INT);
    RETURN_ON_ERROR_NOT_INVALID_PARAM(err_code);
...
uint32_t bsp_btn_ble_init(bsp_btn_ble_error_handler_t error_handler, bsp_event_t * p_startup_bsp_evt)
{
    ...
    if (m_num_connections == 0)
    {
        err_code = smartsock_buttons_configure();
    }
